...
import datetime
from connectors.helper_functions import tick_to_decimals
...
class Candle:
    def __init__(self, candle_info, timeframe, exchange):
        if exchange == "binance":
            ...
            self.timestamp = int(self.timestamp.timestamp() * 1000) # Convert to milliseconds
...
class Contract:
    def __init__(self, contract_info, exchange):
        if exchange == "binance":
            ...
            self.tick_size = 1 / pow(10, contract_info['pricePrecision']) # 1 / 10^pricePrecision
            self.lot_size = 1 / pow(10, contract_info['quantityPrecision']) # 1 / 10^quantityPrecision
